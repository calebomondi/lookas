...
logger = logging.getLogger("🦉Lookas Provider")
cfg = Config()
...
def _cap_requirements(reqs: dict) -> tuple[int, int]:
    analysts = min(reqs.get("max_analysts", 3), 5)
    words = min(reqs.get("word_count", 1000), 1000)
    if reqs.get("max_analysts", 3) > 5:
        logger.warning("Capping max_analysts to 5")
    if reqs.get("word_count", 1000) > 1000:
        logger.warning("Capping word_count to 1000")
    return analysts, words
    ...
    def _on_negotiation(self, event):
        async def _handle():
            try:
                ...
                max_analysts, word_count = _cap_requirements(reqs)
